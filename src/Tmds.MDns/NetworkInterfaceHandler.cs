...
namespace Tmds.MDns
{
    class NetworkInterfaceHandler
    {
        ...
        public NetworkInterfaceHandler(ServiceBrowser serviceBrowser, NetworkInterface networkInterface, IPAddress listenIP)
        {
            ServiceBrowser = serviceBrowser;
            NetworkInterface = networkInterface;
            _index = NetworkInterface.GetIPProperties().GetIPv4Properties().Index;
            this.listenIP = listenIP;
            this.key = CalcKey(_index, this.listenIP);
#if NETSTANDARD1_3
        ...
        private void HandlePacketServiceInfos(EndPoint localEndpoint)
        {
                ...
                else
                {
                    ...
                    if (modified)
                    {
                        if (wasComplete != service.IsComplete)
                        {
                            ...
                            {
                                ServiceBrowser.OnServiceAdded(service, localEndpoint);
                            }
                        }
                        else if (service.IsComplete)
                        {
                            ServiceBrowser.OnServiceChanged(service, localEndpoint);
                        }
                    }
                }
            }
        ...
        public static string CalcKey(int index, IPAddress ip)
        {
            return $"{index}|{ip}";
        }
        ...
        public void Enable()
        {
            ...
            lock (this)
            {
                ...
                if (this.listenIP != null)
                {
                    ...
                    try
                    {
                        _socket.Bind(new IPEndPoint(this.listenIP, IPv4EndPoint.Port));
                        Debug.Write("bind worked");
                        //Debug.Print("bind worked");
                    }
                    catch (Exception)
                    {
...
#endif
        {
            lock (this)
            {
                ...
                if (validPacket)
                {
                    ...
                    Socket s;
#if NETSTANDARD1_3
                    s = args.ConnectSocket;
#else
                    s = _socket;
#endif
                    HandlePacketServiceInfos(s.LocalEndPoint);
                }
                StartReceive();
            }
        }
